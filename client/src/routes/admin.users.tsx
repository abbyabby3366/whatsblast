...
import { Edit, Loader2, Plus, Shield, Store, Trash2, UserRound } from 'lucide-react'
...
function getRows(data: unknown): User[] {
  if (Array.isArray(data)) return data as User[]
  if (data && typeof data === 'object' && 'results' in data) return (data as { results?: User[] }).results ?? []
  return []
}
...
function AdminUsersPage() {
  ...
  const users = getRows(data).filter((user) => user.id !== currentUserId)
  const totalCount = Array.isArray(data) ? users.length : (data as { count?: number } | undefined)?.count || users.length
  const maxPage = Math.max(1, Math.ceil(totalCount / pageSize))
  const refresh = () => { queryClient.invalidateQueries({ queryKey: ['admin', 'users'] }); queryClient.invalidateQueries({ queryKey: ['users'] }) }
  ...
  const quickUpdate = useMutation({ mutationFn: ({ id, patch }: { id: string; patch: Partial<User> }) => api.patch(`users/${id}/`, { json: patch }).json<User>(), onSuccess: refresh, onError: () => toast.error('Unable to update user') })
  const deleteUser = useMutation({ mutationFn: (id: string) => api.delete(`users/${id}/`), onSuccess: () => { refresh(); toast.success('User deleted') }, onError: () => toast.error('Unable to delete user') })
  const bulkDelete = useMutation({ mutationFn: async (ids: string[]) => { await Promise.all(ids.map((id) => api.delete(`users/${id}/`))) }, onSuccess: () => { setSelected([]); refresh(); toast.success('Selected users deleted') }, onError: () => toast.error('Unable to bulk delete users') })
